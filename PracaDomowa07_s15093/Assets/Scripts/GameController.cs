...
public class GameController : MonoBehaviour
{
    ...
    private PlayerMovement playerGameObject;
    ...
    private void LoadLevel(int idx) {
        ...
        for (int row = 0; row < Level.levelArray.Length; row++) {
            char[] charArr= Level.levelArray[row].ToCharArray(); 
            for (int col = 0; col < charArr.Length; col++) {
                InstantiateField(col, row);
            }  
        }
        //for (int row = 0; row < lev.height; row++) {
        //    for (int col = 0; col < lev.width; col++) {
    ...
    private void InstantiateField(int row, int col) {
        //Instantiate() na odpowiednim prefabie w zależności od FloorType i Entity
         Instantiate(floorPref, new Vector3(row, col, 0), Quaternion.identity);
